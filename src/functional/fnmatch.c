...
int main(void)
{
	...
	{
		int r, x;
		rtrace_printf_begin("0xf3d90");
		rtrace_printf(TYPE_ARG, TYPE_POINTER, 0, tests[i].pattern);
		rtrace_printf(TYPE_ARG, TYPE_POINTER, 1, tests[i].string);
		rtrace_printf(TYPE_ARG, TYPE_INT, 2, tests[i].flags);
		r = fnmatch(tests[i].pattern, tests[i].string, tests[i].flags);
		rtrace_printf(TYPE_RET, TYPE_POINTER, 0, r);
		rtrace_printf_end("0xf3d90");
		x = tests[i].expected;
